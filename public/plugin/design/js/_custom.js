    ...
    function($scope, Fabric, FabricConstants, Keypress, $http, $timeout, $mdDialog, $mdToast, Upload, mainFactory, $timeout) {
        ...
        $scope.set_step = function(step, ignore) {
            ...
            if (ignore) {
                ...
                //if (!$scope.campaign_data.sale_png64) alert('Please enter a sale price.');
                if (!$scope.campaign_data.title) alert('Please enter a Product Name to continue.');
        ...
        $scope.estimated_profit = function() {
            ...
            if (goal < 20) {
                bottle = Number($scope.campaign_data.formula.retail);
        ...
        $scope.hasFormula = function(product) {
            var hide_standard = ['Pre-Workout', 'Primal Whey', 'Alcohol Complex', 'Supergreens Complex'];
        ...
        $scope.add_rating_path = function(review_name) {
            review_name = $scope.campaign_data.review_name
            return 'images/ratings/' + review_name.replace(/ /g, '_') + '.jpg';
        }
